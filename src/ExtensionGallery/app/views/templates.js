angular.module('galleryApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/views/home.html',
    "<h1 class=page-header>{{headline}} <a ng-href={{feed}} class=\"btn btn-success\"><span class=\"glyphicon glyphicon-cog\" aria-hidden=true></span> Feed</a></h1><div id=extensions><article ng-repeat=\"package in packages\"><img class=icon ng-src={{package.Icon}} alt=\"{{package.Name}}\"><h3><a ng-href=#/extension/{{package.ID}}>{{package.Name}} - {{package.Version}}</a></h3><p>{{package.Description}}</p><a class=\"btn-sm btn-primary\" ng-href={{package.DownloadUrl}}><span class=\"glyphicon glyphicon-cloud-download\" aria-hidden=true></span> Download</a> <a ng-href=/feed/extension/{{package.ID}} class=\"btn-sm btn-success\"><span class=\"glyphicon glyphicon-cog\" aria-hidden=true></span> Feed</a> <time>{{package.DatePublished | date:'&nbsp;MMM d. yyyy'}}</time></article></div>"
  );
